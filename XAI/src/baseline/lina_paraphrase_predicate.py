...
from src.module.lina import PARAPHRASE_PROMPT_TEMPLATE, HEAD_INSTRUCTION
load_dotenv()
...
def postpreocessing(response):
    matches_statements, matches_predicates, matches_instances, matches_information = get_paraphrase_info(response=response)
    ic(len(matches_statements), len(matches_predicates), len(matches_instances), len(matches_information))
    category_names = ['simplified statement', 'list predicates', 'list instances', 'list information']
    categories = [matches_statements, matches_predicates, matches_instances, matches_information]

    dic_info = {}
    for cat_name, cat in zip (category_names, categories):
        if cat_name in ['list predicates', 'list instances']:
            split = ","
